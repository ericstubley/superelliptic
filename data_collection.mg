...
One can also collect data for large classes of curves at a time using GetData.
*/
...
function FiltrationSequence(f, l)
    ...
    */
    ...
    return [i : i in [1..#indeps] | #indeps[i] gt 0], Gstructure;
end function;
...
function CupProducts(f, l)
    /* 
    Let a_i be a nonzero class in H^1_S(chi^i), where S = {f, infinity} and chi has order gamma = ord_l(q).
    Return the sequence [e_1, ..., e_gamma], where e_i = 0 if a_i cup a_1 = 0 and e_i = 1 otherwise
    */
    ...
    return [IsPower(Evaluate(gioverf, alpha), l) select 0 else 1 : gioverf in gioverflist];
end function;
...
procedure GetData(l, q, d, output_file : filt_sequence := true, auts := false, filter_by_cup_products := [])
    ...
    Data includes:
    - l, q, gamma := order of q in F_l^*, f(t), and the number of monic irreducible polynomials equivalent to f(t);
    - a sequence [e_1, ..., e_gamma], where e_i = 0 if a_i cup b = 0 and e_i = 1 otherwise;
    - if filt_sequence = true: the elementary abelian invariants of the class group of y^l = f(t), 
      the filtration sequence, and the l-rank of the class group;
    - if auts = true: the number of automorphisms of the curve (defined over F_q).
...
// GetData(7, 3, 6, "output.txt" : filt_sequence := false);